...
final class AdminNoticeDetailViewController: UIViewController {
    ...
    var viewModel: AdminNoticeDetailViewModel!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        viewConfigure()
        viewModel = AdminNoticeDetailViewModel()
...
extension AdminNoticeDetailViewController {
    ...
    @objc private func deletedButtonTapped() {
        ...
        alert.addAction(UIAlertAction(title: "확인", style: .default, handler: { _ in
            if let notice = self.noticeInfo {
                self.viewModel.deletedNotice(notice: notice, completion: {
                    self.navigationController?.popViewController(animated: true)
                })
            }
        }))
